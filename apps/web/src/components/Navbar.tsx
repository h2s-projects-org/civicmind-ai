...
export default function Navbar({ user, notifications, onMarkNotificationsRead, onLogout }: NavbarProps) {
  ...
  const [gpuStatus, setGpuStatus] = useState<{ cudfEnabled: boolean; gpuType: string | null } | null>(null);

  const unreadCount = notifications.filter((n) => !n.read).length;

  // Fetch live GPU/system health from the backend
  useEffect(() => {
    fetch("/api/system/health")
      .then((r) => r.json())
      .then((data) => {
        setGpuStatus({
          cudfEnabled: data.cudfEnabled || false,
          gpuType: data.gpuType || null,
        });
      })
      .catch(() => setGpuStatus({ cudfEnabled: false, gpuType: null }));
  }, []);
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-white/5 bg-slate-950/20 px-6 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-4">
        {/* NVIDIA Stack Acceleration Status — Live from Backend */}
        <div className="hidden items-center gap-2 rounded-lg border border-white/5 bg-slate-950/30 px-3 py-1.5 text-xs text-slate-400 lg:flex backdrop-blur-md">
          <Cpu className={`h-4 w-4 ${gpuStatus?.cudfEnabled ? 'text-emerald-500 animate-pulse' : 'text-amber-500'}`} />
          <span className="font-medium text-slate-300 font-display">GPU System:</span>
          <span className={`font-mono ${gpuStatus?.cudfEnabled ? 'text-emerald-400' : 'text-amber-400'}`}>
            {gpuStatus?.cudfEnabled ? 'cuDF Enabled' : 'CPU Fallback (Pandas)'}
          </span>
          <span className={`h-1.5 w-1.5 rounded-full ${gpuStatus?.cudfEnabled ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`}></span>
        </div>
